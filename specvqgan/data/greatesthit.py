...
class GreatestHitWave(torch.utils.data.Dataset):
    ...
    def __init__(self, split, wav_dir, random_crop, mel_num, spec_crop_len, spec_len,
        ...
        # Dabin : Add '_denoised' to video_name
        # video_name = [vidx + '_denoised' for vidx in video_name]
        # print(video_name)
        ...
        # Dabin : Remove '_denoised' from video_name for key in self.video_audio_path
        # self.video_audio_path = {v: os.path.join(self.data_path, v, f'audio/{v}_denoised_resampled.wav') for v in video_name}
        # video_name = [v.replace('_denoised', '') for v in video_name]
        self.video_audio_path = {v: os.path.join(self.data_path, v, f'audio/{v}_denoised_resampled.wav') for v in video_name}
        
        # Test video audio path keys
        # print("Keys in self.video_audio_path:", self.video_audio_path.keys())
    ...
    def __getitem__(self, idx):
        ...
        video_idx = self.dataset[idx]
        video, start_idx = video_idx.split('_')
        # Dabin : Add '_denoised' to video
        # video = video + '_denoised'
        ...
        # print(video, start_idx)
        wave_path = self.video_audio_path[video]        # error
